...
Stores raw vectors in SQLite as blobs so the index can be rebuilt
after deletions. Replaces ChromaDB entirely.
"""
...
class FaissStore:
    ...
    def __init__(self, path: str, dimension: int = _DEFAULT_DIM):
        ...
        # SQLite DB — stores metadata AND vector blobs
    ...
    def search(self, query_emb: np.ndarray, n_results: int = 10,
        ...
        """Search for similar vectors. Returns (ids, texts, distances, metadatas)."""
    ...
    def delete(self, ids: Optional[list[str]] = None, where: Optional[dict] = None) -> None:
        """Delete items and rebuild FAISS index from remaining vectors."""
        with self._lock:
            if ids:
                placeholders = ",".join("?" * len(ids))
    ...
    def _init_db(self) -> None:
        ...
        self._db.execute("""
            CREATE TABLE IF NOT EXISTS docs (
                ...
                vector BLOB,          -- numpy float32 embedding stored as blob
                rowid INTEGER
            )
        """)
        self._db.execute("""
        ...
        """)
        self._db.execute("""
            CREATE INDEX IF NOT EXISTS idx_pos_doc ON pos_map(doc_id)
        """)
        self._db.commit()
    ...
    def _write_docs(self, ids: list[str], texts: list[str],
        ...
        for i, (doc_id, text, meta) in enumerate(zip(ids, texts, metadatas)):
            ...
            self._db.execute(
                "INSERT OR REPLACE INTO docs (id, text, metadata, vector, rowid) "
                "VALUES (?, ?, ?, ?, ?)",
    ...
    def _get_rows(self, faiss_indices: list[int]) -> list[tuple]:
        ...
        # Build ORDER BY to match input index order
        order_clause = " ".join(
            f"WHEN {pos} THEN {i}" for i, pos in enumerate(faiss_indices)
        )
        cur = self._db.execute(
    ...
    def _rebuild_index(self) -> None:
        ...
        for i, doc_id in enumerate(doc_ids):
            ...
            self._db.execute(
                "UPDATE docs SET rowid = ? WHERE id = ?",
                (i, doc_id)
            )
    ...
    def _matches_where(self, metadata: dict, where: dict) -> bool:
        """Check if metadata matches a Chroma-style where clause."""
    ...
    def _where_to_sql(self, where: dict, prefix: str = "") -> tuple[str, list]:
        """Convert Chroma-style where to SQL conditions."""
        conditions = []
        params = []

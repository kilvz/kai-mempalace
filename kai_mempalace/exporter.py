...
def export_palace(palace_path: str, output_dir: str, format: str = "markdown") -> dict:
    ...
    Args:
        palace_path: Path to the FAISS palace directory.
    ...
    offset = 0
    while offset < total:
        batch = palace.list_drawers(limit=1000, offset=offset)
        if not batch:
            break
            ...
            batch_grouped[wing][room].append(
                {
                    ...
                    "filed_at": drawer.get("created_at", ""),
                    "added_by": drawer.get("metadata", {}).get("added_by", ""),
                }
            )
        ...
        for wing, rooms in batch_grouped.items():
            ...
            for room, drawers in rooms.items():
                ...
                with _safe_open_for_write(room_path, "a" if not is_new else "w") as f:
                    ...
                    for drawer_group in drawers:
                        source = drawer_group["source"] or "unknown"
                        filed = drawer_group["filed_at"] or "unknown"
                        added_by = drawer_group["added_by"] or "unknown"

                        f.write(
                            f"## {drawer_group['id']}\n"
                            f"\n"
                            f"> {_quote_content(drawer_group['content'])}\n"
                            f"\n"
                            f"| Field | Value |\n"
                            f"|-------|-------|\n"
                            f"| Source | {source} |\n"
